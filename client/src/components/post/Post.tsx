...
const Post = ({_id,user,tags,title,text}:PostType) => {
    return (
        <div>
            <div className={s.newsItemWrapper} key={_id} >
                ...
                <div className={s.newsInfo}>
                    ...
                    <div className={s.newsInfoItem}>{user.fullName}</div>
                    {/*<div className={s.newsInfoItem}>{m.categories}</div>*/}
                    <div className={s.newsInfoItem}>{tags}</div>
                </div>
                <Link to={`/news/${_id}`} className={s.newsTitle}>{title}</Link>
                <p className={s.newsText}>{text.substring(0, 300) + '...'}</p>
